...
class SourceConstructor:
    ...
    def random_split(self, df, n_sources=10):
        ...
        idx_splits = np.array_split(df_shuffled.index.to_numpy(), n_sources)
        splits = [df_shuffled.loc[idx].copy() for idx in idx_splits]
    ...
    #     print("Generating high penalty sources...")
    ...
    #         ur_dict = self.UR
    #     if len(ur_columns) == 1:
    #         return self.random_split(self.T, n_sources=n_sources)
    ...
    def high_penalty_sources(self, n_sources):
        """
        FAST (bulk) high-penalty construction.

        Idea:
      - Find "perfect" rows that satisfy the UR on all UR columns.
      - Find "donor" rows that are outside the UR-value universe on all UR columns.
      - For m = min(#perfect, #donor), remove those m perfect rows and replace each
        with 2 "corrupted" copies that break UR co-occurrence by swapping one UR
        attribute each from a donor row.
      - Finally, random-split the mutated table into n_sources sources.
        """
        T = self.T.copy()
        ...
        if isinstance(self.UR, pd.DataFrame):
            ur_columns = list(self.UR.columns)
            ur_dict = {c: set(self.UR[c].dropna().unique()) for c in ur_columns}
        else:
            ur_columns = list(self.UR.keys())
            ur_dict = {c: set(v) for c, v in self.UR.items()}

        # Keep only columns that exist in T
        ur_columns = [c for c in ur_columns if c in T.columns]

        # If <2 columns, co-occurrence breaking isn't meaningful
        if len(ur_columns) < 2:
            return self.random_split(T, n_sources=n_sources)

        # ---- masks (vectorized) ----
        perfect_mask = pd.Series(True, index=T.index)
        for c in ur_columns:
            perfect_mask &= T[c].isin(ur_dict[c])

        ur_values_flat = set().union(*ur_dict.values())

        donor_mask = pd.Series(True, index=T.index)
        for c in ur_columns:
            donor_mask &= ~T[c].isin(ur_values_flat)

        perfect_idx = T.index[perfect_mask]
        donor_idx   = T.index[donor_mask]

        m = min(len(perfect_idx), len(donor_idx))
        if m == 0:
            # Nothing to mutate
            return self.random_split(T, n_sources=n_sources)

        # Take first m of each (deterministic given T order)
        perfect = T.loc[perfect_idx[:m]].copy()
        donors  = T.loc[donor_idx[:m]].copy()

        # Corrupt using first two UR columns (same intent as old code)
        c0, c1 = ur_columns[0], ur_columns[1]

        row1 = perfect.copy()
        row2 = perfect.copy()
        row1[c1] = donors[c1].to_numpy()
        row2[c0] = donors[c0].to_numpy()

        # Remove mutated perfect rows and append corrupted rows (bulk)
        T_mut = T.drop(index=perfect_idx[:m])
        T_mut = pd.concat([T_mut, row1, row2], ignore_index=True)

        return self.random_split(T_mut, n_sources=n_sources)
...
#OLD VERSION OF SOURCE CONSTRUCTORS - KEPT FOR REFERENCE
# import pandas as pd
# import numpy as np
# import random
# from itertools import product
...
#         # if <2 attrs, just random split (same spirit as your fast method)
#         if len(ur_columns) < 2:
#             return self.random_split(T, n_sources=n_sources)

#         ur_values_flat = set().union(*ur_dict.values())

#         # ---------- estimate mutation count fast ----------
#         perfect_mask = pd.Series(True, index=T.index)
#         for c in ur_columns:
#             perfect_mask &= T[c].isin(ur_dict[c])
...
#             row1 = perfect.copy()
#             row2 = perfect.copy()
#             row1[c1] = donors[c1].values
#             row2[c0] = donors[c0].values
